...
import { ConditionType, defaultCondition } from '@/constants/condition.schema';
import { useRegFormContext } from './reducer';
import FormButtons from './FormButtons';
import { motion } from 'framer-motion';

const Condition = ({ fieldsSchema, title, description }: any) => {
  const { state, dispatch } = useRegFormContext();
  const form = useForm<ConditionType>({
    resolver: zodResolver(conditions),
    defaultValues: {
  ...
  const collectForm = async () => {
    ...
    dispatch({
      type: 'ADD_RESOURCE',
      resource: 'conditions',
      payload: form.getValues('conditions'),
    });
    dispatch({ type: 'NEXT' });
  };
        ...
        <div className="min-h-[60vh]">
          ...
          {fields.map((field, index) => (
            ...
            >
              ...
              <ReusableForm collectionName="conditions" fieldSchema={fieldsSchema} index={index} />
              <Button
                size={'sm'}
                type="button"
                onClick={() => {
          ...
          <div className="flex items-center justify-between mt-4 mr-4">
            <Button
              type="button"
              size="sm"
              onClick={() => append(defaultCondition.conditions[0])}
