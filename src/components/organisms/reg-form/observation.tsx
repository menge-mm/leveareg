...
import { observations } from '@/constants';
import { zodResolver } from '@hookform/resolvers/zod';
import { ObservationType, defaultObservation } from '@/constants/observation.schema';
import { useRegFormContext } from './reducer';
import FormButtons from './FormButtons';
import { motion } from 'framer-motion';

const Observation = ({ fieldsSchema, index, title, description }: any) => {
  const { dispatch, state } = useRegFormContext();

  const form = useForm<ObservationType>({
    resolver: zodResolver(observations),
    defaultValues: {
  ...
  const collectForm = async () => {
    ...
    dispatch({
      ...
      payload: form.getValues('observations'),
    });
    dispatch({ type: 'NEXT' });
  };
        ...
        <div className="min-h-[60vh]">
          ...
          {fields.map((field, index) => (
            ...
            >
              ...
              <ReusableForm collectionName="observations" fieldSchema={fieldsSchema} index={index} />
              <Button
                size={'sm'}
                type="button"
                onClick={() => {
          ...
          <div className="flex items-center justify-between mt-4 mr-4">
            <Button
              type="button"
              size="sm"
              onClick={() => append(defaultObservation.observations[0])}
